...
const CategorySidebar = ({ categories }) => {
  ...
  const activeTypes = types?.filter(type => type.category_id === hoveredCategory?.id) || [];
  ...
  return (
    ...
    >
      ...
      <aside className="w-full bg-white/70 backdrop-blur-2xl rounded-[32px] border border-white/40 shadow-[0_8px_32px_rgba(0,0,0,0.04)] flex flex-col h-[480px] z-20 relative overflow-hidden">
        ...
        <div className="p-3 bg-white/30 backdrop-blur-md border-t border-white/40">
          <Link href="/categories" className="flex items-center justify-center gap-2 w-full py-3 rounded-2xl bg-white/60 border border-white/80 text-[11px] font-black text-slate-500 uppercase tracking-[0.2em] hover:bg-white hover:text-blue-600 transition-all shadow-sm active:scale-95 group">
            Explorer All Nodes 
            ...
            >
              ...
              <ul className="space-y-1 overflow-y-auto custom-scrollbar px-1 pb-3">
                {activeTypes.map((type, idx) => (
                  <li key={type.id}>
                    <Link 
                      href={`/category/${slugify(hoveredCategory.name)}/${slugify(type.name)}`}
                      className="
              ...
              <div className="mt-auto m-1.5 p-3.5 bg-slate-900/90 backdrop-blur-2xl rounded-[24px] flex items-center justify-between shadow-2xl">
                <div className="flex items-center gap-2">
                   ...
                   <span className="text-[9px] font-black text-white uppercase tracking-widest">Active Nodes</span>
                </div>
