...
const BrandCategorySection = () => {
  ...
  const handleCategoryClick = (categoryId) => {
    const path = userId ? `/user/${userId}/category/${categoryId}` : `/category/${categoryId}`;
    router.push(path);
  };
  ...
  // Define the base responsive class for scroll items
  // Base (small screens): 3 items (w-[calc(33.33%-16px)])
  // md: 4 items (w-[calc(25%-18px)])
  // lg: 5 items (w-[calc(20%-20px)])
  const responsiveScrollItemClass = 
    "flex-shrink-0 snap-start transition duration-300 cursor-pointer p-2 " + 
    "w-[calc(33.33%-16px)] " + // Mobile (approx 3 items, compensating for space-x-6)
    "md:w-[calc(25%-18px)] " +  // Tablet (approx 4 items)
    "lg:w-[calc(20%-20px)]";   // Desktop (approx 5 items)
  ...
  return (
    <div className="w-full rounded-3xl bg-white p-6 shadow-md">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        ...
        {/* 🚀 FEATURED STORES SECTION - RESPONSIVE SCROLL 🚀 */}
        <div>
          <div className="flex items-center justify-between mb-6">
            ...
            <motion.button
              ...
              onClick={() => {
                if (!userId) {
                  console.warn("User not logged in. Cannot navigate to user-specific store page.");
                  return; 
                }
          ...
          {hasStores ? (
            <div 
              className="flex space-x-6 overflow-x-scroll pb-4 scrollbar-hide snap-x"
        ...
        {/* ------------------------------------------- */}
        {/* TOP CATEGORIES SECTION - RESPONSIVE SCROLL */}
        {/* ------------------------------------------- */}
        <div>
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-semibold text-gray-800">
          ...
          {hasCategories ? (
            ...
            >
              {categories.map((category) => (
                <motion.div
                  key={category.id}
                  // Applying the same responsive class for consistency
          ...
          {/* Custom CSS to hide the scrollbar for a smoother look */}
          <style jsx global>{`
