...
export default function ProductActions({
  quantity, setQuantity, handleAddToCart, handleFavouriteClick, isFavourited,
}) {
  return (
    <div className="mt-10 space-y-6">
      <div className="flex flex-wrap items-end gap-6">
        ...
        <button 
          ...
          className={`h-[64px] px-5 rounded-2xl border transition-all ${isFavourited ? 'bg-red-500 border-red-500 text-white' : 'bg-white border-slate-200 text-slate-400 hover:text-red-500'}`}
        >
          <Heart className={`w-6 h-6 ${isFavourited ? "fill-white" : ""}`} />
        </button>
      </div>
    </div>
  );
