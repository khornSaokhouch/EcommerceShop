...
const DeletePaymentModal = ({ isOpen, onClose, payment, onConfirm }) => {
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-[32px] p-8 w-full max-w-sm relative z-10 shadow-2xl border border-slate-100 text-center">
          ...
          <h2 className="text-xl font-black text-slate-900 mb-2 uppercase tracking-tight">Delete Payment Account?</h2>
          <p className="text-[13px] text-slate-500 mb-8 leading-relaxed font-medium">
             Removing <span className="text-slate-900 font-bold underline decoration-red-500">{payment.account_name}</span> will permanently delete this payment account.
          </p>
          <div className="grid grid-cols-2 gap-3">
            ...
            <button onClick={onConfirm} className="py-4 text-[13px] font-bold uppercase text-white bg-red-500 rounded-2xl hover:bg-red-600 shadow-xl shadow-red-200">Delete</button>
          </div>
        </motion.div>
      </div>
    </AnimatePresence>
