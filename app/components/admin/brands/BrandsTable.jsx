...
const BrandsTable = ({ brands, search, onEdit, onDelete, onToggleStatus }) => {
      ...
      <table className="w-full text-left border-collapse">
        <thead className="bg-slate-50/50">
          <tr>
            ...
            <th className="px-8 py-5 text-[13px] font-medium text-slate-500 uppercase tracking-widest">Brand Name</th>
            ...
            <tr key={brand.id} className="hover:bg-slate-50/30 transition-colors group">
              ...
              <td className="px-8 py-5">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-[13px] font-bold text-slate-900 uppercase tracking-tight">{brand.name}</span>
                  </div>
              </td>
              <td className="px-8 py-5">
                  <button 
              ...
              <td className="px-8 py-5 text-right">
                  ...
                  <button onClick={() => onEdit(brand)} className="p-2.5 bg-slate-50 text-slate-400 hover:text-blue-600 hover:bg-white hover:shadow-lg rounded-xl transition-all border border-transparent hover:border-blue-100"><Edit size={16} /></button>
                  <button onClick={() => onDelete(brand.id)} className="p-2.5 bg-slate-50 text-slate-400 hover:text-red-500 hover:bg-white hover:shadow-lg rounded-xl transition-all border border-transparent hover:border-red-100"><Trash2 size={16} /></button>
                </div>
              </td>
            </tr>
          ))}
    ...
    return (
        <div className="py-32 text-center flex flex-col items-center gap-4">
            ...
            <p className="text-[13px] font-medium text-slate-500 uppercase tracking-widest">
                {search ? `No brand found for "${search}"` : "Brand registry empty"}
            </p>
        </div>
    )
}
