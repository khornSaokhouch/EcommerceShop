...
export default function ProductsPage() {
  ...
  if (loading && products.length === 0) return (
    <div className="h-[60vh] flex flex-col items-center justify-center bg-white">
        ...
        <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Decrypting Inventory Grid...</p>
    </div>
  );
    ...
    <div className="space-y-8 pb-20 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-400 text-[11px] font-bold uppercase tracking-widest mt-1">Manage global hardware deployment registry</p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50/50">
              <tr>
                ...
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Hardware Node</th>
              ...
              {displayedProducts.length === 0 ? (
                <tr>
                  <td colSpan="6" className="py-24 text-center">
                    ...
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Registry Entry Empty</p>
                  </td>
                </tr>
              ) : (
    ...
    return (
        <div className="relative group min-w-[200px]">
            ...
            <select
                ...
                className="w-full appearance-none bg-slate-50 border-none rounded-2xl py-3.5 pl-11 pr-10 text-[10px] font-black uppercase tracking-widest text-slate-700 outline-none focus:ring-4 focus:ring-blue-500/5 transition-all cursor-pointer"
            >
      ...
      <td className="px-8 py-5">
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0">
            <p className="text-[13px] font-black text-slate-900 uppercase tracking-tight truncate max-w-[250px]">{product.name}</p>
            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">SKU: {product.id}</p>
          </div>
        </div>
      </td>
      <td className="px-8 py-5">
        <div className="space-y-1">
            <p className="text-[10px] font-black text-blue-600 uppercase tracking-tight">{categoryName}</p>
            <p className="text-[9px] font-bold text-slate-400 uppercase">{storeName}</p>
        </div>
      </td>
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-[32px] p-8 w-full max-w-sm relative z-10 shadow-2xl border border-slate-100 text-center">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button onClick={onConfirm} disabled={loading} className="py-4 text-xs font-black uppercase text-white bg-red-500 rounded-2xl hover:bg-red-600 shadow-xl shadow-red-200 transition-all flex items-center justify-center">
