...
export default function BrandsPage() {
  const {
    brands, loading, fetchBrands, saveBrand, deleteBrand, toggleBrandStatus
  } = useBrandStore()
  ...
  const handleSave = async (data) => {
    try {
      ...
      setIsFormOpen(false)
      toast.success("Brand Registry Updated")
    } catch (err) {
  ...
  const handleDelete = async () => {
    try {
      ...
      toast.success("Brand Classification Purged")
    } catch (err) {
  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
                ...
                <h1 className="text-3xl font-black text-slate-900 uppercase tracking-tighter leading-none">Brands Management</h1>
                <p className="text-slate-500 text-[13px] font-medium uppercase tracking-widest mt-2">Manage hardware brand classifications</p>
            </div>
            <button
        ...
        <div className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm flex flex-col justify-center gap-6">
            <div className="flex items-center justify-between">
                <div>
                    <p className="text-[11px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-1">Total Brands</p>
      ...
      <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm min-h-[400px]">
         {loading ? (
           <div className="py-32 flex flex-col items-center gap-4">
              ...
              <p className="text-[13px] font-medium text-slate-500 uppercase tracking-widest">Scanning Brand Registry</p>
           </div>
         ) : (
           <BrandsTable
              brands={brands}
      ...
      <DeleteBrandModal
        ...
        brand={brands.find(b => b.id === deleteConfirmId)}
